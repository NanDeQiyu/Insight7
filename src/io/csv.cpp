...
static const std::vector<DType> dt_order = {
    ...
    DType::U16, DType::U32, DType::U64, DType::F32, DType::F64, DType::C32,
    DType::C64,

    // Partially supported (storage only for now)
    DType::F16, DType::BF16, DType::F8_E4M3, DType::F8_E5M2};
...
static bool is_kernel_registered(const std::string &op_name, DeviceKind device,
                                 DType dtype) {
  return insight_has_kernel(op_name.c_str(), static_cast<int32_t>(device),
