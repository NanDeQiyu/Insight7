...
PrintOptions &get_print_options() { return g_print_opts; }

void set_printoptions(int precision, int threshold, int edgeitems,
                      int linewidth, bool sci_mode) {
  if (precision > 0)
    g_print_opts.precision = precision;
  if (threshold > 0)
    g_print_opts.threshold = threshold;
  if (edgeitems > 0)
    g_print_opts.edgeitems = edgeitems;
  if (linewidth > 0)
    g_print_opts.linewidth = linewidth;
...
template <typename T> std::string format_scalar(T value) {
  std::ostringstream oss;
  if constexpr (std::is_same_v<T, float> || std::is_same_v<T, double>) {
    if (g_print_opts.sci_mode) {
      oss << std::scientific << std::setprecision(g_print_opts.precision)
          << value;
    } else {
    ...
    if (g_print_opts.sci_mode) {
      ...
      if (im >= 0) {
        ...
        oss << "(" << std::fixed << std::setprecision(prec) << re << std::fixed
            << std::setprecision(prec) << im << "j)";
      }
    }
  } else {
    oss << value;
...
template <typename T> std::string format_row(const T *data, int64_t size) {
  std::ostringstream oss;
  oss << "[";
  ...
  if (size > 2 * g_print_opts.edgeitems) {
    ...
    for (int i = size - g_print_opts.edgeitems; i < size; ++i) {
      if (i > size - g_print_opts.edgeitems)
        oss << ", ";
      oss << format_scalar(data[i]);
    }
  } else {
...
template <typename T>
std::string format_array_recursive(const T *data, const Shape &shape, int dim,
  ...
  for (int64_t i = 0; i < actual_size; ++i) {
    ...
    elements.push_back(format_array_recursive(data + src_idx * stride, shape,
                                              dim + 1, indent + 1, summary));
  }
...
std::string to_string(const Array &arr, int indent) {
  ...
  // Build final string
  std::ostringstream oss;
  oss << g_print_opts.prefix << "(shape=" << shape << ", dtype=" << dtype
      << ", place=" << place << ",\n";
  oss << std::string(indent, ' ') << data_str << ")";
