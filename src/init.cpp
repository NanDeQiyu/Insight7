...
namespace ins {
    ...
    static void unload_library(LibHandle lib) {
        FreeLibrary(lib);
    }
     ...
     * @param lib_name   Library name without extension (e.g., "insight_cpu_backend")
     */
    static void load_backend_plugin(DeviceKind kind, const char* lib_name) {
        ...
        InitPluginFunc init_plugin = reinterpret_cast<InitPluginFunc>(get_symbol(lib, kind == DeviceKind::GPU ? "InitPluginGPU" : "InitPluginCPU"));
        if (!init_plugin) {
            unload_library(lib);
            INS_THROW("get_symbol function failed: fail to get function " + std::string(kind == DeviceKind::GPU ? "InitPluginGPU" : "InitPluginCPU"));
        }
     ...
     */
    static bool init_static_backend(DeviceKind kind, C_Status(*init_plugin)(CustomRuntimeParams*)) {
        CustomRuntimeParams params;
        std::memset(&params, 0, sizeof(params));
        params.size = sizeof(CustomRuntimeParams);
        params.interface = new C_DeviceInterface();
