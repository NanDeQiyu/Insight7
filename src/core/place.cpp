...
extern "C" {
    ...
    C_Status insight_get_available_devices(char*** devices_out) {
        ...
        if (ins::has_device(ins::DeviceKind::GPU)) {
            const C_DeviceInterface* iface = ins::get_device_interface(ins::DeviceKind::GPU);
        ...
        char** devices = (char**)std::malloc((count + 1) * sizeof(char*));  // +1 for NULL terminator
        if (!devices) {
            return C_FAILED;
        }
...
namespace ins {
    ...
    const C_DeviceInterface* get_device_interface(DeviceKind kind) {
        ...
        INS_CHECK(idx < 2 && idx >= 0, "Invalid DeviceKind: ", static_cast<int>(kind));
    ...
    void set_device_interface(DeviceKind kind, const C_DeviceInterface* iface,
        ...
        if (idx < 2) {
            ...
            g_device_infos[idx].device_type_name = device_type_name ? device_type_name :
    ...
    size_t device_count(DeviceKind kind) {
        const C_DeviceInterface* iface = get_device_interface(kind);
        if (!iface || !iface->get_device_count) return 0;
    ...
    Place GPUPlace(int device_id) {
        ...
        INS_CHECK(device_id >= 0 && static_cast<size_t>(device_id) < device_count(DeviceKind::GPU),
    ...
    Place::Place(DeviceKind kind, int device_id)
        ...
        INS_CHECK(device_id >= 0,
            "Place: device_id must be non-negative, got ", device_id);
    }
    ...
    void* Place::allocate(size_t size) const {
        ...
        INS_CHECK(status == C_SUCCESS && ptr != nullptr,
            "GPU allocation failed for ", size, " bytes on device ", device_id_);
        return ptr;
    }
    ...
    void Place::copy_from_host(void* dst, const void* src, size_t size) const {
        ...
        INS_CHECK(status == C_SUCCESS,
            "Device host-to-device copy failed on device ", device_id_);
    }
    ...
    void Place::copy_to_host(void* dst, const void* src, size_t size) const {
        ...
        INS_CHECK(status == C_SUCCESS,
            "Device-to-host copy failed on device ", device_id_);
    }
    ...
    void Place::copy_on_device(void* dst, const void* src, size_t size) const {
        ...
        INS_CHECK(status == C_SUCCESS,
            "Device-to-device copy failed on device ", device_id_);
    }
