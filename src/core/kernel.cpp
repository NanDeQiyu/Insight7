...
C_Status insight_register_kernel(const char *op_name, int32_t device_type,
  ...
  if (!kernel) {
    kernel_error_message =
        std::string("insight_register_kernel: kernel is null for operator '") +
        op_name + "'";
    insight_set_last_error(kernel_error_message.c_str());
    return C_FAILED;
  }
  ...
  if (device_type != INSIGHT_DEVICE_CPU && device_type != INSIGHT_DEVICE_GPU) {
    kernel_error_message =
        ...
        std::to_string(device_type) + " for operator '" + op_name + "'";
    insight_set_last_error(kernel_error_message.c_str());
    return C_FAILED;
  }
  ...
  if (dtype <= INSIGHT_DTYPE_UNKNOWN || dtype >= INSIGHT_DTYPE_COUNT) {
    kernel_error_message =
        ...
        std::to_string(dtype) + " for operator '" + op_name + "'";
    insight_set_last_error(kernel_error_message.c_str());
    return C_FAILED;
  }
...
C_Status insight_kernel_launch(const char *op_name, int32_t device_type,
  ...
  if (!inputs) {
    kernel_error_message =
        std::string(
            "insight_kernel_launch: inputs array is null for operator '") +
        op_name + "'";
    insight_set_last_error(kernel_error_message.c_str());
    return C_FAILED;
  }
  ...
  if (!outputs) {
    kernel_error_message =
        std::string(
            "insight_kernel_launch: outputs array is null for operator '") +
        op_name + "'";
    insight_set_last_error(kernel_error_message.c_str());
    return C_FAILED;
  }
  ...
  if (!kernel) {
    kernel_error_message =
        std::string("insight_kernel_launch: kernel not found for operator '") +
  ...
  if (status == C_FALLBACK && device_type == INSIGHT_DEVICE_GPU) {
    return insight_kernel_launch(op_name, INSIGHT_DEVICE_CPU, dtype, inputs,
                                 outputs);
  }
  ...
  if (status != C_SUCCESS && status != C_FALLBACK) {
    kernel_error_message =
        ...
        ", dtype=" + std::to_string(dtype) + " with status " +
