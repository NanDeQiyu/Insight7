...
void Slice::normalize(int64_t dim_size, int64_t &start_out, int64_t &stop_out,
                      int64_t &step_out) const {
  step_out = step;
  if (step_out == 0) {
...
Slice Slice::compose(const Slice &inner) const {
  ...
  // Compose stop (simplified - for full implementation, need to compute based
  // on sizes)
...
Slice parse_slice(const std::string &spec) {
  ...
  auto parse_part = [&](const std::string &part,
                        int idx) -> std::optional<int64_t> {
