...
Array as_complex(const Array &x) {
  ...
  INS_CHECK(has_complex_shape(x),
            "as_complex: input must have last dimension = 2");
  INS_CHECK(
      x.dtype() == DType::F32 || x.dtype() == DType::F64,
      "as_complex: input must be float32 or float64, but got input dtype: ",
      dtype_name(x.dtype()));
...
Array real(const Array &z) {
  ...
  INS_CHECK(is_cmplx, "real: input must be complex, but got dtype: ",
            dtype_name(z.dtype()));
...
Array imag(const Array &z) {
  ...
  bool is_cmplx = is_complex(z);
  INS_CHECK(is_cmplx, "imag: input must be complex, but got dtype: ",
            dtype_name(z.dtype()));
  Array real_view = as_real(z);
  int last_dim = real_view.shape().ndim() - 1;
