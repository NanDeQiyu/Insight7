...
namespace ins {
    ...
    Array trace(const Array& x) {
        ...
        ops().launch("trace", work.place(), working_dtype,
            { result.layout_ptr(), work.layout_ptr() },
            { result.layout_ptr() });
    ...
    std::tuple<Array, Array> lu(const Array& x, bool pivot) {
        ...
        ops().launch("lu", work.place(), working_dtype,
    ...
    std::tuple<Array, Array, Array> lu_unpack(const Array& LU, const Array& pivots) {
        ...
        ops().launch("lu_unpack", work.place(), working_dtype,
            { (void*)P.layout_ptr(), (void*)L.layout_ptr(), (void*)U.layout_ptr(), (void*)work.layout_ptr(), (void*)pivots.layout_ptr() },
            { P.layout_ptr(), L.layout_ptr(), U.layout_ptr() });

        return { to_original_dtype(P, LU.dtype()),
                 to_original_dtype(L, LU.dtype()),
    ...
    Array cholesky_solve(const Array& A, const Array& B, bool lower) {
        ...
        ops().launch("cholesky_solve", A_work.place(), working_dtype,
            { result.layout_ptr(), A_work.layout_ptr(), B_work.layout_ptr(), &lower_flag },
            { result.layout_ptr() });
    ...
    std::tuple<Array, Array, Array> svd(const Array& x, bool full_matrices) {
        ...
        ops().launch("svd", work.place(), working_dtype,
            { U.layout_ptr(), S.layout_ptr(), VT.layout_ptr(), work.layout_ptr(), &full_flag },
    ...
    std::tuple<Array, Array> eig(const Array& x) {
        ...
        ops().launch("eig", work.place(), working_dtype,
    ...
    std::tuple<Array, Array> eigh(const Array& x, const std::string& uplo) {
        ...
        ops().launch("eigh", work.place(), working_dtype,
            { eigenvalues.layout_ptr(), eigenvectors.layout_ptr(), work.layout_ptr(), &uplo_code },
            { eigenvalues.layout_ptr(), eigenvectors.layout_ptr() });
    ...
    Array lstsq(const Array& A, const Array& B, double rcond) {
        ...
        int nrhs = (B_work.shape().ndim() == 1) ? 1 : static_cast<int>(B_work.shape().dim(1));

        Shape out_shape = (B_work.shape().ndim() == 1) ? Shape({ n }) : Shape({ n, nrhs });
        Array result(out_shape, working_dtype, A_work.place());

        ops().launch("lstsq", A_work.place(), working_dtype,
    ...
    Array solve_triangular(const Array& A, const Array& B, bool lower, bool unit_diag) {
        ...
        ops().launch("solve_triangular", A_work.place(), working_dtype,
            { result.layout_ptr(), A_work.layout_ptr(), B_work.layout_ptr(), &lower_flag, &unit_flag },
            { result.layout_ptr() });
