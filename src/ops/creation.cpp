...
Array eye(int64_t n, int64_t m, int64_t k, DType dtype, const Place &place) {
  ...
  Shape shape({n, m});
  Array result(shape, dtype, place);
  ops().launch("eye", place, dtype, {result.layout_ptr(), &k},
               {result.layout_ptr()});
  return result;
}
...
Array arange(double start, double end, double step, DType dtype,
  ...
  ops().launch("arange", place, dtype, {result.layout_ptr(), &start, &step},
               {result.layout_ptr()});
  return result;
}
...
Array linspace(double start, double stop, int64_t num, DType dtype,
               const Place &place) {
  Array result(Shape({num}), dtype, place);
  ops().launch("linspace", place, dtype, {result.layout_ptr(), &start, &stop},
               {result.layout_ptr()});
  return result;
}
