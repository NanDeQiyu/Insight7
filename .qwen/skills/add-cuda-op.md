...
## 10. 动态输出分配（kernel 内部 cudaMalloc）

某些算子（如 `unique`、`nonzero`）的输出大小在 kernel 运行前未知，需要 kernel 内部 `cudaMalloc` 分配。

**前端必须用默认构造 `Array()` 做占位符，不能用 `Array(Shape({0}), ...)`：**

```cpp
// ✅ 正确 — 默认构造，ref_count = nullptr
Array result;
ops().launch("myop", place, dtype,
    {x.layout_ptr(), result.layout_ptr()},
    {result.layout_ptr()});
return Array(result.layout_ptr());

// ❌ 错误 — 显式构造设置 ref_count，干扰 allocate_gpu_output 生命周期
Array result(Shape({0}), dtype, place);
```

**kernel 内部 `allocate_gpu_output` 模板：**
