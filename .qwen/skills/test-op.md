...
`.to(CPUPlace())` 返回临时 `Array` 对象，`data<T>()` 返回其内部指针。如果临时对象在语句结束时析构，指针悬垂，读到垃圾数据。

```cpp
// ❌ 错误 — 临时对象析构后 data 悬垂
const float *data = c.to(CPUPlace()).data<float>();
EXPECT_FLOAT_EQ(data[0], 1.0f);  // 读到垃圾

// ✅ 正确 — 命名变量持有生命周期
Array cpu_c = c.to(CPUPlace());
const float *data = cpu_c.data<float>();
EXPECT_FLOAT_EQ(data[0], 1.0f);  // 正确

// ✅ 也可以用 helper 函数（内部已有命名变量）
expect_float_equal_gpu<float>(c, {1.0f, 2.0f, 3.0f});
```

**症状**：垃圾值如 `1.0578383e+24`、`3.0687035e-41`，或 `Device-to-host copy failed`。
**调试技巧**：如果多个不相关的 kernel 测试都出现类似垃圾值，优先检查是否是悬垂指针而非 kernel 逻辑错误。
