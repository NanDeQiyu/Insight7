...
extern "C" {
     ...
     * @return C_SUCCESS on success, C_FALLBACK to request CPU fallback, or error code
     */
     ...
     * the CPU kernel is invoked, and data is transferred back to the original device.
     * If no kernel is found on either device, C_FAILED is returned.
     *
     * @param op_name     Operator name
