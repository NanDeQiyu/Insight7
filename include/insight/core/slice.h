...
struct Slice {
  ...
  Slice(std::optional<int64_t> start, std::optional<int64_t> stop,
        int64_t step = 1)
  ...
  // Normalize with dimension size (returns normalized start, stop, step, and
  // length)
  void normalize(int64_t dim_size, int64_t &start_out, int64_t &stop_out,
