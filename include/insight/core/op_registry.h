...
namespace ins {
    ...
    public:
         ...
         */
        static void launch(const std::string& op_name,
            const Place& place,
            DType dtype,
         ...
         * @return true if a kernel is registered
         */
        static bool has_kernel(const std::string& op_name, DeviceKind device, DType dtype) {
            int32_t device_type = (device == DeviceKind::CPU) ? INSIGHT_DEVICE_CPU : INSIGHT_DEVICE_GPU;
            return insight_has_kernel(op_name.c_str(), device_type, static_cast<int32_t>(dtype)) != 0;
        }
         ...
         */
        static std::vector<DType> list_kernels(const std::string& op_name, DeviceKind device) {
