 ...
 */
Array ones(const Shape &shape, DType dtype = DType::F32,
           const Place &place = get_device());

/**
 * @brief Create an array filled with a constant value.
 */
Array full(const Shape &shape, double fill_value, DType dtype = DType::F32,
           const Place &place = get_device());

/**
 * @brief Create an identity matrix (2D square with ones on diagonal).
 */
Array eye(int64_t n, int64_t m = -1, int64_t k = 0, DType dtype = DType::F32,
          const Place &place = get_device());
 ...
 */
Array arange(double end, DType dtype = DType::I64,
             const Place &place = get_device());
Array arange(double start, double end, double step = 1.0,
             DType dtype = DType::I64, const Place &place = get_device());

/**
 * @brief Create a 1D array with linearly spaced values.
 */
Array linspace(double start, double stop, int64_t num, DType dtype = DType::F32,
               const Place &place = get_device());

/**
 * @brief Create a 1D array with logarithmically spaced values.
 */
Array logspace(double start, double stop, int64_t num, double base = 10.0,
 ...
 */
template <typename T>
Array to_array(std::initializer_list<T> data,
               const Place &place = get_device()) {
 ...
 */
template <typename T>
Array to_array(const T *data, const Shape &shape,
               const Place &place = get_device()) {
  Array result(shape, dtype_of<T>(), ins::CPUPlace());
  T *dst = result.data<T>();
 ...
 */
template <typename T>
Array to_array(const T *data, const Shape &shape, DType dtype,
               const Place &place = get_device()) {
  if (dtype_of<T>() == dtype) {
    Array result(shape, dtype, ins::CPUPlace());
    T *dst = result.data<T>();
 ...
 */
template <typename T>
Array to_array(const std::vector<T> &data, const Shape &shape, DType dtype,
               const Place &place = get_device()) {
  INS_CHECK(shape.numel() == static_cast<int64_t>(data.size()),
            "to_array: shape.numel() must equal data.size()");
 ...
 */
template <typename T>
Array to_array(std::initializer_list<T> data, const Shape &shape,
               const Place &place = get_device()) {
  std::vector<T> vec(data);
...
/**
 * @brief Create an array from an initializer list with explicit shape and
 * dtype.
 */
template <typename T>
Array to_array(std::initializer_list<T> data, const Shape &shape, DType dtype,
               const Place &place = get_device()) {
  std::vector<T> vec(data);
...
/**
 * @brief Create a zero-filled array with same shape/dtype/place as another
 * array.
 */
Array zeros_like(const Array &arr);

/**
 * @brief Create a one-filled array with same shape/dtype/place as another
 * array.
 */
