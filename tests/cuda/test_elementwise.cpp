...
// ============================================================================
namespace {
template <typename T> void fill_sequential_cpu(Array &cpu_arr) {
...
template <typename T> void fill_constant_gpu(Array &gpu_arr, T val) {
  Array cpu_arr(gpu_arr.shape(), gpu_arr.dtype(), CPUPlace());
  T *data = cpu_arr.data<T>();
  int64_t n = cpu_arr.numel();
  for (int64_t i = 0; i < n; ++i) {
...
template <typename T>
void expect_complex_equal_gpu(const Array &gpu_arr,
