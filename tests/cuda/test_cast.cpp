...
template<typename T>
void expect_float_values(const Array& input, const std::vector<T>& expected, T tol = 1e-6) {
...
template<typename T>
void expect_complex_values(const Array& input, const std::vector<std::complex<T>>& expected, T tol = 1e-6) {
    Array arr = input.to(ins::CPUPlace());
    ASSERT_EQ(arr.numel(), static_cast<int64_t>(expected.size()));
...
TEST_F(CastTestGPU, IdentityCast) {
    ...
    for (int64_t i = 0; i < 6; ++i) {
        EXPECT_FLOAT_EQ(dst.to(ins::CPUPlace()).data<float>()[i], src.to(ins::CPUPlace()).data<float>()[i]);
    }
}