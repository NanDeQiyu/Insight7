...
class TestOperatorCUDA:
    ...
    def test_add_operator(self):
        ...
        np.testing.assert_allclose(
            to_numpy(to_gpu(a_np) + to_gpu(b_np)), a_np + b_np, rtol=1e-10)
    ...
    def test_negation(self):
        a_np = np.array([1, -2, 3], dtype=np.float64)
        np.testing.assert_allclose(
            to_numpy(-to_gpu(a_np)), -a_np, rtol=1e-10)

    def test_scalar_add(self):
        a_np = np.array([1, 2, 3], dtype=np.float64)
        np.testing.assert_allclose(
            to_numpy(to_gpu(a_np) + 10.0), a_np + 10.0, rtol=1e-10)

    def test_scalar_mul(self):
        a_np = np.array([1, 2, 3], dtype=np.float64)
        np.testing.assert_allclose(
            to_numpy(to_gpu(a_np) * 2.0), a_np * 2.0, rtol=1e-10)
