...
TEST(DTypeTest, CAPICompatibility) {
    ...
    EXPECT_STREQ(insight_dtype_name(static_cast<int32_t>(DType::C64)), "complex128");

    // 验证大小
    EXPECT_EQ(insight_dtype_size(static_cast<int32_t>(DType::F32)), sizeof(float));
    EXPECT_EQ(insight_dtype_size(static_cast<int32_t>(DType::C64)), sizeof(std::complex<double>));
