...
--   LD_LIBRARY_PATH=build/backends/cpu ~/.luarocks/bin/busted tests/cpu/lua/test_signal_filtering.lua

describe("Signal Filtering CPU Tests", function()
  local ins
  setup(function()
    ins = require("insight")
  end)
  ...
  it("detrend_constant", function()
    ...
    assert.near(-2.0, y:get(0), 1e-10)
    assert.near(0.0, y:get(2), 1e-10)
    assert.near(2.0, y:get(4), 1e-10)
  end)
  ...
  it("lfilter_fir_basic", function()
    ...
    assert.near(1.0, y:get(0), 1e-10)
    assert.near(1.0, y:get(1), 1e-10)
    for i = 2, 5 do
      assert.near(0.0, y:get(i), 1e-10)
    end
  end)
  ...
  it("lfilter_fir_moving_avg", function()
    ...
    assert.is_not_nil(y)
    assert.near(1.0, y:get(1), 1e-10)
    assert.near(2.0, y:get(2), 1e-10)
    assert.near(3.0, y:get(3), 1e-10)
    assert.near(4.0, y:get(4), 1e-10)
  end)
  ...
  it("lfilter_iir_impulse", function()
    ...
    assert.is_not_nil(y)
    assert.near(1.0, y:get(0), 1e-10)
    assert.near(0.5, y:get(1), 1e-10)
    assert.near(0.25, y:get(2), 1e-10)
    assert.near(0.125, y:get(3), 1e-10)
    assert.near(0.0625, y:get(4), 1e-10)
  end)
  ...
  it("resample_identity", function()
    ...
    assert.are.equal(5, y.numel)
    for i = 0, 4 do
      assert.near(i + 1.0, y:get(i), 1e-6)
    end
  end)
  ...
  it("resample_poly_identity", function()
    ...
    assert.are.equal(5, y.numel)
    for i = 0, 4 do
      assert.near(i + 1.0, y:get(i), 1e-10)
    end
  end)
  ...
  it("firfilter_basic", function()
    ...
    assert.near(1.0, y:get(0), 1e-10)
    assert.near(2.0, y:get(1), 1e-10)
    assert.near(1.0, y:get(2), 1e-10)
    for i = 3, 6 do
      assert.near(0.0, y:get(i), 1e-10)
    end
  end)
