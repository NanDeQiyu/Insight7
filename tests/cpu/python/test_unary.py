...
class TestUnaryCPU:
    ...
    def test_sin_cos_tan(self):
        ...
        np.testing.assert_allclose(
            ins.sin(ins.from_numpy(a_np)).numpy(), np.sin(a_np), rtol=1e-10)
        np.testing.assert_allclose(
            ins.cos(ins.from_numpy(a_np)).numpy(), np.cos(a_np), rtol=1e-10)
        np.testing.assert_allclose(
            ins.tan(ins.from_numpy(a_np)).numpy(), np.tan(a_np), rtol=1e-10)
