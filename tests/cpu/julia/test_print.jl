# Print CPU tests — aligned with C++ test_print.cpp
# Run with:
#   LD_LIBRARY_PATH=build/backends/cpu julia tests/cpu/julia/test_print.jl
...
# show_2d
a = Insight.ones(Int64[3, 4], Insight.float64)
s = sprint(show, a)
check("show_2d", length(s) > 0)

# show_int
a = Insight.zeros(Int64[3], Insight.int32)
s = sprint(show, a)
...
# show_2d_data
a = Insight.from_data(Float64[1 2; 3 4])
s = sprint(show, a)
check("show_2d_data", length(s) > 0)

# show_does_not_crash_large
a = Insight.ones(Int64[100, 100], Insight.float32)
s = sprint(show, a)
check("show_large", length(s) > 0)

# show_empty
a = Insight.zeros(Int64[0], Insight.float32)
s = sprint(show, a)
