...
static bool approx_equal(float a, float b, float rtol = 1e-5f, float atol = 1e-6f) {
    return std::abs(a - b) <= atol + rtol * std::abs(b);
}
...
static bool check_matrix_equal(const Array& A, const Array& B, double rtol = 1e-5) {
    ...
    if (A.dtype() == DType::F64) {
        ...
        for (int64_t i = 0; i < n; ++i) {
            if (!approx_equal(a[i], b[i], static_cast<float>(rtol))) return false;
        }
    }
    else {
        return false;
...
TEST_F(LinalgTest, Cholesky3x3) {
    ...
    {
        Array A = create_matrix_f32(3, 3, { 4.0f, 12.0f, -16.0f, 12.0f, 37.0f, -43.0f, -16.0f, -43.0f, 98.0f });
        Array L = cholesky(A, true);
        Array LLT = matmul(L, transpose(L));
...
TEST_F(LinalgTest, QR3x3) {
    ...
    // F32 测试
    {
        Array A = create_matrix_f32(3, 3, { 12.0f, -51.0f, 4.0f, 6.0f, 167.0f, -68.0f, -4.0f, 24.0f, -41.0f });
        auto [Q, R] = qr(A, "reduced");
        Array QR = matmul(Q, R);
...
TEST_F(LinalgTest, Eigh3x3) {
    ...
    {
        Array A = create_matrix_f32(3, 3, { 2.0f, -1.0f, 0.0f, -1.0f, 2.0f, -1.0f, 0.0f, -1.0f, 2.0f });
        auto [vals, vecs] = eigh(A, "L");
...
TEST_F(LinalgTest, MatrixRankFull) {
    ...
    {
        Array A = create_matrix_f32(3, 3, { 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f });
        Array r = matrix_rank(A);
        EXPECT_EQ(r.item<int64_t>(), 2);
    }
}
...
TEST_F(LinalgTest, SolveTriangularUpper) {
    ...
    {
        Array U = create_matrix_f32(3, 3, { 1.0f, 2.0f, 3.0f, 0.0f, 4.0f, 5.0f, 0.0f, 0.0f, 6.0f });
...
TEST_F(LinalgTest, SolveTriangularLower) {
    ...
    {
        Array L = create_matrix_f32(3, 3, { 2.0f, 0.0f, 0.0f, 1.0f, 2.0f, 0.0f, 1.0f, 1.0f, 2.0f });
        Array b = create_vector_f32({ 4.0f, 5.0f, 6.0f });
        Array x = solve_triangular(L, b, true, false);
...
TEST_F(LinalgTest, Cov) {
    ...
    {
        std::vector<float> data_f32 = { 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f };
