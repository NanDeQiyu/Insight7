...
extern "C" {
    ...
    C_Status contiguous_copy_gpu(void** inputs, void** outputs) {
        ...
        if (insight_array_is_contiguous(in)) {
            ...
            cudaError_t err = cudaMemcpy(out->data, in->data, bytes, cudaMemcpyDeviceToDevice);
            if (err != cudaSuccess) {
        ...
        cudaMemcpy(d_strides, in->strides, ndim * sizeof(int64_t), cudaMemcpyHostToDevice);
        ...
        contiguous_copy_kernel << <blocks, threads >> > (
            ...
            static_cast<const char*>(in->data) + in->offset * elem_size,
            d_dims,
            d_strides,
            total,
            ndim,
...
REGISTER_GPU_KERNEL(contiguous_copy, INSIGHT_DTYPE_F8_E4M3, contiguous_copy_gpu);
REGISTER_GPU_KERNEL(contiguous_copy, INSIGHT_DTYPE_F8_E5M2, contiguous_copy_gpu);