...
extern "C" {
    ...
    C_Status full_kernel_gpu(void** inputs, void** outputs) {
        ...
        case INSIGHT_DTYPE_F64: {
            full_kernel << <blocks, threads >> > (static_cast<double*>(out->data), fill_val, n);
            break;
        }
        case INSIGHT_DTYPE_C32: {
            std::complex<float> val(static_cast<float>(fill_val), 0.0f);
            full_kernel << <blocks, threads >> > (static_cast<std::complex<float>*>(out->data), val, n);
            break;
        }
        case INSIGHT_DTYPE_C64: {
            std::complex<double> val(fill_val, 0.0);
            full_kernel << <blocks, threads >> > (static_cast<std::complex<double>*>(out->data), val, n);
            break;
        }
        default:
            gpu_set_last_error(("full_kernel_gpu: unsupported dtype " + std::to_string(dtype)).c_str());
            return C_FAILED;
        }
