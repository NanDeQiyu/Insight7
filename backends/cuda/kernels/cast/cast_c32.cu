...
C_Status cast_c32_kernel_gpu(void** inputs, void** outputs) {
    ...
    switch (target_dtype) {
        case INSIGHT_DTYPE_BOOL: {
            cast_c32_to_bool_kernel<<<blocks, threads>>>(src_data, (bool*)dst->data, n);
            break;
        }        case INSIGHT_DTYPE_F32: {
            cast_c32_to_f32_kernel<<<blocks, threads>>>(src_data, (float*)dst->data, n);
            break;
        }        case INSIGHT_DTYPE_F64: {
            cast_c32_to_f64_kernel<<<blocks, threads>>>(src_data, (double*)dst->data, n);
            break;
        }        case INSIGHT_DTYPE_C64: {
            cast_c32_to_c64_kernel<<<blocks, threads>>>(src_data, (cuDoubleComplex*)dst->data, n);
            break;
        }
        default:
...
REGISTER_GPU_KERNEL(cast, INSIGHT_DTYPE_C32, cast_c32_kernel_gpu);
