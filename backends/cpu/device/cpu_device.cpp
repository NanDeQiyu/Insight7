...
extern "C" {
    ...
    static C_Status cpu_initialize(void) {
        return C_SUCCESS;
    }

    static C_Status cpu_finalize(void) {
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_deinit_device(C_Device device) {
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_set_device(C_Device device) {
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_synchronize_device(C_Device device) {
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_device_memory_allocate(C_Device device, void** ptr, size_t size) {
    ...
    static C_Status cpu_device_memory_deallocate(C_Device device, void* ptr, size_t size) {
        if (ptr) std::free(ptr);
        return C_SUCCESS;
    }

    static C_Status cpu_host_memory_allocate(C_Device device, void** ptr, size_t size) {
        return cpu_device_memory_allocate(device, ptr, size);
    }

    static C_Status cpu_host_memory_deallocate(C_Device device, void* ptr, size_t size) {
        return cpu_device_memory_deallocate(device, ptr, size);
    }

    static C_Status cpu_memory_copy_h2d(C_Device device, void* dst, const void* src, size_t size) {
        if (!dst || !src) return C_FAILED;
        std::memcpy(dst, src, size);
        return C_SUCCESS;
    }

    static C_Status cpu_memory_copy_d2h(C_Device device, void* dst, const void* src, size_t size) {
        return cpu_memory_copy_h2d(device, dst, src, size);
    }

    static C_Status cpu_memory_copy_d2d(C_Device device, void* dst, const void* src, size_t size) {
        return cpu_memory_copy_h2d(device, dst, src, size);
    }
    ...
    static C_Status cpu_device_memory_set(C_Device device, void* ptr, unsigned char value, size_t size) {
        if (!ptr || size == 0) return C_SUCCESS;
        std::memset(ptr, value, size);
        return C_SUCCESS;
    }

    static C_Status cpu_device_memory_stats(C_Device device, size_t* total_memory, size_t* free_memory) {
    ...
    static C_Status cpu_async_memory_copy_p2p(C_Device dst_device, C_Device src_device,
    ...
    static C_Status cpu_stream_wait_event(C_Device device, C_Stream stream, C_Event event) {
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_record_event(C_Device device, C_Stream stream, C_Event event) {
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_get_compute_capability(C_Device device, size_t* capability) {
    ...
    static C_Status cpu_get_multi_process(C_Device device, size_t* multi_process) {
        if (multi_process) *multi_process = 0;
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_get_max_threads_per_block(C_Device device, size_t* threads) {
        if (threads) *threads = 0;
        return C_SUCCESS;
    }
    ...
    static C_Status cpu_profiler_create(C_Profiler* prof, const char* name, int device_id) {
        if (prof) *prof = nullptr;
        return C_SUCCESS;
    }
    ...
    const char* cpu_get_last_error(void) {
        return cpu_last_error_str.empty() ? "CPU backend: no error" : cpu_last_error_str.c_str();
    }
