...
#pragma omp parallel for
    for (int64_t linear = 0; linear < total; ++linear) {
        ...
        int64_t src_offset = cpu_indices_to_linear(ndim, dims, src_strides, indices);
...
C_Status flip_kernel_cpu(void** inputs, void** outputs) {
    ...
    switch (x->dtype) {
        ...
        case INSIGHT_DTYPE_F64:
            cpu_flip_impl<double>(
                (const double*)x->data, (double*)out->data,
                ndim, dims, src_strides, dst_strides, axis);
            break;
        case INSIGHT_DTYPE_C32:
            cpu_flip_impl<std::complex<float>>(
                (const std::complex<float>*)x->data, (std::complex<float>*)out->data,
                ndim, dims, src_strides, dst_strides, axis);
            break;
        case INSIGHT_DTYPE_C64:
            cpu_flip_impl<std::complex<double>>(
                (const std::complex<double>*)x->data, (std::complex<double>*)out->data,
                ndim, dims, src_strides, dst_strides, axis);
            break;
        default:
