...
template <typename T>
static void concat_impl(const std::vector<const InsightArray *> &inputs, T *dst,
                        int64_t ndim, const int64_t *out_dims,
                        const int64_t *out_strides, int axis) {
...
C_Status concat_kernel_cpu(void **inputs, void **outputs) {
  ...
  case INSIGHT_DTYPE_BOOL:
    concat_impl<bool>(in_arrays, (bool *)out->data, ndim, out_dims, out_strides,
                      axis);
    break;
  case INSIGHT_DTYPE_U8:
    concat_impl<uint8_t>(in_arrays, (uint8_t *)out->data, ndim, out_dims,
                         out_strides, axis);
    break;
  case INSIGHT_DTYPE_I8:
    concat_impl<int8_t>(in_arrays, (int8_t *)out->data, ndim, out_dims,
                        out_strides, axis);
    break;
  case INSIGHT_DTYPE_I16:
    concat_impl<int16_t>(in_arrays, (int16_t *)out->data, ndim, out_dims,
                         out_strides, axis);
    break;
  case INSIGHT_DTYPE_I32:
    concat_impl<int32_t>(in_arrays, (int32_t *)out->data, ndim, out_dims,
                         out_strides, axis);
    break;
  case INSIGHT_DTYPE_I64:
    concat_impl<int64_t>(in_arrays, (int64_t *)out->data, ndim, out_dims,
                         out_strides, axis);
    break;
  ...
  case INSIGHT_DTYPE_U32:
    concat_impl<uint32_t>(in_arrays, (uint32_t *)out->data, ndim, out_dims,
                          out_strides, axis);
    break;
  case INSIGHT_DTYPE_U64:
    concat_impl<uint64_t>(in_arrays, (uint64_t *)out->data, ndim, out_dims,
                          out_strides, axis);
    break;
  case INSIGHT_DTYPE_F32:
    concat_impl<float>(in_arrays, (float *)out->data, ndim, out_dims,
                       out_strides, axis);
    break;
  case INSIGHT_DTYPE_F64:
    concat_impl<double>(in_arrays, (double *)out->data, ndim, out_dims,
                        out_strides, axis);
    break;
  case INSIGHT_DTYPE_C32:
    concat_impl<std::complex<float>>(in_arrays,
                                     (std::complex<float> *)out->data, ndim,
                                     out_dims, out_strides, axis);
    break;
  case INSIGHT_DTYPE_C64:
    concat_impl<std::complex<double>>(in_arrays,
