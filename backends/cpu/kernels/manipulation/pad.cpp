 ...
 *   inputs[2] = int64_t* pad_width (2 * ndim values: before0, after0, before1, after1, ...)
 *   inputs[3] = double* constant_value
 *
...
static inline void compute_out_strides(int64_t ndim, const int64_t* dims, int64_t* strides) {
    if (ndim == 0) return;
    strides[ndim - 1] = 1;
    ...
    C_Status pad_kernel_cpu(void** inputs, void** outputs) {
        ...
        case INSIGHT_DTYPE_BOOL:
            pad_impl<bool>(
                ...
                ndim, in_dims, out_dims, pad_before,
                (constant_value != 0));
            break;
