// backends/cpu/kernels/cast/cast_f64.cpp
/**
 * @file cast_f64.cpp
 * @brief CPU kernel for casting from f64 to all types.
 *
...
C_Status cast_f64_kernel_cpu(void** inputs, void** outputs) {
    ...
    if (!src || !dst) {
        cpu_set_last_error("cast_f64: null array pointer");
        return C_FAILED;
    }
    ...
    switch (target_dtype) {
        case INSIGHT_DTYPE_BOOL: {
            ...
            CAST_LOOP(n, d[i] = std::complex<double>(s[i], 0.0););
            break;
        }
        default:
            cpu_set_last_error("cast_f64: unsupported target type");
            return C_FAILED;
    }
...
REGISTER_CPU_KERNEL(cast, INSIGHT_DTYPE_F64, cast_f64_kernel_cpu);
