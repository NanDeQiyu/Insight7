// backends/cpu/kernels/cast/cast_i32.cpp
/**
 * @file cast_i32.cpp
 * @brief CPU kernel for casting from i32 to all types.
 *
...
C_Status cast_i32_kernel_cpu(void **inputs, void **outputs) {
  ...
  if (!src || !dst) {
    cpu_set_last_error("cast_i32: null array pointer");
    return C_FAILED;
  }
  ...
  case INSIGHT_DTYPE_C32: {
    ...
    CAST_LOOP(n, d[i] = std::complex<float>(static_cast<float>(s[i]), 0.0f););
    break;
  }
  ...
  default:
    cpu_set_last_error("cast_i32: unsupported target type");
    return C_FAILED;
  }
...
REGISTER_CPU_KERNEL(cast, INSIGHT_DTYPE_I32, cast_i32_kernel_cpu);
