...
C_Status cast_u8_kernel_cpu(void **inputs, void **outputs) {
  ...
  if (!src || !dst) {
    cpu_set_last_error("cast_u8: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    cpu_set_last_error("cast_u8: unsupported target type");
    return C_FAILED;
  }
...
REGISTER_CPU_KERNEL(cast, INSIGHT_DTYPE_U8, cast_u8_kernel_cpu);
