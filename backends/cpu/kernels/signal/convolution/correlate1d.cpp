// backends/cpu/kernels/signal/convolution/correlate1d.cpp
// CPU kernel for 1D direct correlation
...
C_Status correlate1d_kernel_cpu(void **inputs, void **outputs) {
  ...
  if (!x || !h || !out) {
    cpu_set_last_error("correlate1d: null array pointer");
    return C_FAILED;
  }
...
#pragma omp parallel for if (out_len > 1000)
    ...
    cpu_set_last_error("correlate1d: unsupported dtype, need F32 or F64");
    return C_FAILED;
  }
...
REGISTER_CPU_KERNEL(correlate1d, INSIGHT_DTYPE_F64, correlate1d_kernel_cpu);
REGISTER_CPU_KERNEL(correlate1d, INSIGHT_DTYPE_F32, correlate1d_kernel_cpu);
