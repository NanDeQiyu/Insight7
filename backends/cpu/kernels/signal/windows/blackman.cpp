...
#pragma omp parallel for if (M > 1000)
    ...
    for (int64_t i = 0; i < M; ++i) {
      float angle = scale * i;
      data[i] = 0.42f - 0.5f * cosf(angle) + 0.08f * cosf(2.0f * angle);
    }
  } else {
    cpu_set_last_error("blackman: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_CPU_KERNEL(blackman, INSIGHT_DTYPE_F32, blackman_kernel_cpu);
