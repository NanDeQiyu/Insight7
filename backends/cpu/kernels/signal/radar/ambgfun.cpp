...
// ambgfun kernel
// inputs: [0]=x_data (complex, normalized), [1]=y_data (complex,
// normalized),
//         [2]=params (float array: [fs, N, M])
// outputs: [0]=ambg (doppler_len x delay_len)
//
// The frontend converts input to complex, normalizes, and passes parameters.
C_Status ambgfun_kernel_cpu(void **inputs, void **outputs) {
  ...
  if (out_arr->dtype == INSIGHT_DTYPE_F64) {
...
#pragma omp parallel for if (doppler_len > 32)
  ...
  } else if (out_arr->dtype == INSIGHT_DTYPE_F32) {
    const std::complex<float> *x_data =
        (const std::complex<float> *)x_arr->data;
    const std::complex<float> *y_data =
        (const std::complex<float> *)y_arr->data;
    const float *params = (const float *)params_arr->data;
    float *ambg = (float *)out_arr->data;
...
#pragma omp parallel for if (doppler_len > 32)
    for (int64_t fd = 0; fd < doppler_len; ++fd) {
      float freq = (fd - doppler_len / 2.0f) * fs / doppler_len;
      float omega = 2.0f * (float)M_PI * freq / fs;

      for (int64_t tau = 0; tau < delay_len; ++tau) {
        int64_t tau_shift = tau - (M - 1);

        std::complex<float> sum_val(0.0f, 0.0f);
        for (int64_t n = 0; n < N; ++n) {
          int64_t m = n - tau_shift;
          if (m >= 0 && m < M) {
            float phase = omega * n;
    ...
    cpu_set_last_error("ambgfun: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_CPU_KERNEL(ambgfun, INSIGHT_DTYPE_F32, ambgfun_kernel_cpu);
