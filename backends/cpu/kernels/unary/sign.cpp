...
C_Status sign_kernel_cpu(void** inputs, void** outputs) {
    ...
    switch (x->dtype) {
        ...
        case INSIGHT_DTYPE_I8:
            UNARY_KERNEL_LOOP(int8_t, [](int8_t v) { return v > 0 ? 1 : (v < 0 ? -1 : 0); });
            break;
        case INSIGHT_DTYPE_I16:
            UNARY_KERNEL_LOOP(int16_t, [](int16_t v) { return v > 0 ? 1 : (v < 0 ? -1 : 0); });
            break;
        case INSIGHT_DTYPE_I32:
            UNARY_KERNEL_LOOP(int32_t, [](int32_t v) { return v > 0 ? 1 : (v < 0 ? -1 : 0); });
            break;
        case INSIGHT_DTYPE_I64:
            UNARY_KERNEL_LOOP(int64_t, [](int64_t v) { return v > 0 ? 1 : (v < 0 ? -1 : 0); });
            break;
        case INSIGHT_DTYPE_F32:
            UNARY_KERNEL_LOOP(float, [](float v) { return v > 0 ? 1.0f : (v < 0 ? -1.0f : 0.0f); });
            break;
        case INSIGHT_DTYPE_F64:
            UNARY_KERNEL_LOOP(double, [](double v) { return v > 0 ? 1.0 : (v < 0 ? -1.0 : 0.0); });
            break;
