...
put_along_axis_impl(T *dst, const int64_t *idx_data, const T *val_data,
  ...
  for (int64_t linear = 0; linear < total_indices; ++linear) {
    ...
    // Calculate the output offset: replace the coordinates on axis with pos,
    // and use the values ​​in coord for other dimensions. Note: coord is
    // the coordinate of idx. The shape of idx may be smaller than out. Non-axis
    // dimensions need to be broadcast.
    int64_t out_offset = 0;
    for (int d = 0; d < out_ndim; ++d) {
      if (d == axis) {
        out_offset += pos * out_strides[d];
      } else {
        // For non-axis dimensions, if idx has this dimension, use the value in
        // coord, otherwise use 0
        int64_t c = 0;
        if (d < idx_ndim) {
          c = coord[d];
        }
