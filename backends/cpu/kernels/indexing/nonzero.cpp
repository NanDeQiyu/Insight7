...
static inline bool is_zero_bf16(uint16_t val) {
    return (val & 0x7FFF) == 0;
}
...
template<typename T>
static inline bool is_zero(const T& val) {
    return val == T(0);
}

template<>
inline bool is_zero(const std::complex<float>& val) {
    return val.real() == 0.0f && val.imag() == 0.0f;
}

template<>
inline bool is_zero(const std::complex<double>& val) {
...
template<typename T>
static void nonzero_count_impl(
    const T* src,
    int64_t total,
    int64_t ndim,
    const int64_t* dims,
...
#define NONZERO_DISPATCH(CTYPE) \
    do { \
        nonzero_count_impl<CTYPE>( \
            (const CTYPE*)x->data, total, ndim, dims, strides, nz_count); \
        if (nz_count > 0) { \
            int64_t out_dims[2] = { ndim, nz_count }; \
            allocate_output(out, sizeof(int64_t), INSIGHT_DTYPE_I64, 2, out_dims, ndim * nz_count); \
            nonzero_fill_impl<CTYPE>( \
                (const CTYPE*)x->data, (int64_t*)out->data, total, ndim, dims, strides, nz_count); \
        } else { \
...
extern "C" {
    ...
    C_Status nonzero_kernel_cpu(void** inputs, void** outputs) {
        ...
        switch (x->dtype) {
        case INSIGHT_DTYPE_BOOL:   NONZERO_DISPATCH(bool); break;
        case INSIGHT_DTYPE_U8:     NONZERO_DISPATCH(uint8_t); break;
        case INSIGHT_DTYPE_U16:    NONZERO_DISPATCH(uint16_t); break;
        case INSIGHT_DTYPE_U32:    NONZERO_DISPATCH(uint32_t); break;
        case INSIGHT_DTYPE_U64:    NONZERO_DISPATCH(uint64_t); break;
        case INSIGHT_DTYPE_I8:     NONZERO_DISPATCH(int8_t); break;
        case INSIGHT_DTYPE_I16:    NONZERO_DISPATCH(int16_t); break;
        case INSIGHT_DTYPE_I32:    NONZERO_DISPATCH(int32_t); break;
        case INSIGHT_DTYPE_I64:    NONZERO_DISPATCH(int64_t); break;
        case INSIGHT_DTYPE_F16: {
            ...
            for (int64_t linear = 0; linear < total; ++linear) {
                ...
                if (!is_zero_f16(src[offset])) ++nz_count;
            }
            if (nz_count > 0) {
                int64_t out_dims[2] = { ndim, nz_count };
                allocate_output(out, sizeof(int64_t), INSIGHT_DTYPE_I64, 2, out_dims, ndim * nz_count);
        ...
        case INSIGHT_DTYPE_BF16: {
            ...
            for (int64_t linear = 0; linear < total; ++linear) {
                ...
                if (!is_zero_bf16(src[offset])) ++nz_count;
            }
            // 填充逻辑类似...
            if (nz_count > 0) {
                int64_t out_dims[2] = { ndim, nz_count };
        ...
        case INSIGHT_DTYPE_F32:    NONZERO_DISPATCH(float); break;
        case INSIGHT_DTYPE_F64:    NONZERO_DISPATCH(double); break;
        case INSIGHT_DTYPE_F8_E4M3: {
            ...
            for (int64_t linear = 0; linear < total; ++linear) {
                ...
                if (!is_zero_f8_e4m3(src[offset])) ++nz_count;
            }
            if (nz_count > 0) {
                int64_t out_dims[2] = { ndim, nz_count };
                allocate_output(out, sizeof(int64_t), INSIGHT_DTYPE_I64, 2, out_dims, ndim * nz_count);
        ...
        case INSIGHT_DTYPE_F8_E5M2: {
            ...
            for (int64_t linear = 0; linear < total; ++linear) {
                ...
                if (!is_zero_f8_e5m2(src[offset])) ++nz_count;
            }
            if (nz_count > 0) {
                int64_t out_dims[2] = { ndim, nz_count };
                allocate_output(out, sizeof(int64_t), INSIGHT_DTYPE_I64, 2, out_dims, ndim * nz_count);
        ...
        case INSIGHT_DTYPE_C32:    NONZERO_DISPATCH(std::complex<float>); break;
        case INSIGHT_DTYPE_C64:    NONZERO_DISPATCH(std::complex<double>); break;
        default:
