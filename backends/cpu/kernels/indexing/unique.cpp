...
extern "C" {
    ...
    static void* allocate_output(InsightArray* out, size_t elem_size, int32_t dtype, int64_t numel) {
...
#define UNIQUE_IMPL(CTYPE) \
    do { \
        ...
        for (int64_t i = 0; i < n; ++i) { \
            if (i > 0 && pairs[i].first != pairs[i-1].first) unique_idx++; \
        ...
        \
        CTYPE* dst_unique = (CTYPE*)allocate_output(unique_out, sizeof(CTYPE), x->dtype, unique_count); \
        ...
        if (return_indices) { \
            int64_t* dst_indices = (int64_t*)allocate_output( \
                (InsightArray*)outputs[out_idx++], sizeof(int64_t), INSIGHT_DTYPE_I64, unique_count); \
            if (!dst_indices && unique_count > 0) { \
                cpu_set_last_error("unique: memory allocation failed for indices"); \
                return C_FAILED; \
            } \
            for (int64_t i = 0; i < unique_count; ++i) dst_indices[i] = first_occ[i]; \
        } \
        \
        if (return_inverse) { \
            int64_t* dst_inverse = (int64_t*)allocate_output( \
                (InsightArray*)outputs[out_idx++], sizeof(int64_t), INSIGHT_DTYPE_I64, n); \
            if (!dst_inverse && n > 0) { \
                cpu_set_last_error("unique: memory allocation failed for inverse"); \
                return C_FAILED; \
            } \
            for (int64_t i = 0; i < n; ++i) dst_inverse[i] = inverse[i]; \
        } \
        \
        if (return_counts) { \
            ...
            for (int64_t i = 0; i < unique_count; ++i) dst_counts[i] = counts[i]; \
        } \
    } while(0)
    ...
    do { \
        ...
        std::sort(pairs.begin(), pairs.end(), \
            [](const auto& a, const auto& b) { \
                ...
                if (a_norm == 0 && b_norm == 0) return false; \
        ...
        for (int64_t i = 0; i < n; ++i) { \
            if (i > 0) { \
                uint16_t prev_norm = (pairs[i-1].first & 0x7FFF); \
                uint16_t curr_norm = (pairs[i].first & 0x7FFF); \
                if (!(prev_norm == 0 && curr_norm == 0) && pairs[i].first != pairs[i-1].first) { \
                    unique_idx++; \
                } \
            } \
            inverse[pairs[i].second] = unique_idx; \
