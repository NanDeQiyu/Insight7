...
#pragma omp parallel for
    for (int64_t linear = 0; linear < total_out; ++linear) {
        ...
        if (pos < 0) pos += out_dims[axis];  // Note: uses out_dims, but should use src dims?
...
#define TAKE_DISPATCH(CTYPE) \
    do { \
        if (!has_axis) { \
            take_flat_impl<CTYPE>( \
                (CTYPE*)out->data, out->offset, out->strides[0], \
                (const CTYPE*)x->data, x->offset, x->strides[0], \
                idx_data, n, x->numel); \
        } else { \
            take_axis_impl<CTYPE>( \
                (CTYPE*)out->data, out->offset, out->strides, \
                (const CTYPE*)x->data, x->offset, x->strides, \
                idx_data, n, ndim, out_dims, normalized_axis, idx->numel); \
        } \
    } while(0)
...
extern "C" {
    ...
    C_Status take_kernel_cpu(void** inputs, void** outputs) {
        ...
        switch (x->dtype) {
        case INSIGHT_DTYPE_BOOL:   TAKE_DISPATCH(bool); break;
        case INSIGHT_DTYPE_U8:     TAKE_DISPATCH(uint8_t); break;
        case INSIGHT_DTYPE_I8:     TAKE_DISPATCH(int8_t); break;
        case INSIGHT_DTYPE_I16:    TAKE_DISPATCH(int16_t); break;
        case INSIGHT_DTYPE_I32:    TAKE_DISPATCH(int32_t); break;
        case INSIGHT_DTYPE_I64:    TAKE_DISPATCH(int64_t); break;
        case INSIGHT_DTYPE_U16:    TAKE_DISPATCH(uint16_t); break;
        case INSIGHT_DTYPE_U32:    TAKE_DISPATCH(uint32_t); break;
        case INSIGHT_DTYPE_U64:    TAKE_DISPATCH(uint64_t); break;
        case INSIGHT_DTYPE_F16:    TAKE_DISPATCH(uint16_t); break;
        case INSIGHT_DTYPE_BF16:   TAKE_DISPATCH(uint16_t); break;
        case INSIGHT_DTYPE_F32:    TAKE_DISPATCH(float); break;
        case INSIGHT_DTYPE_F64:    TAKE_DISPATCH(double); break;
        case INSIGHT_DTYPE_C32:    TAKE_DISPATCH(std::complex<float>); break;
        case INSIGHT_DTYPE_C64:    TAKE_DISPATCH(std::complex<double>); break;
        case INSIGHT_DTYPE_F8_E4M3: TAKE_DISPATCH(uint8_t); break;
        case INSIGHT_DTYPE_F8_E5M2: TAKE_DISPATCH(uint8_t); break;
        default:
