        ...
        case INSIGHT_DTYPE_F32:
            if (replace) {
                ...
                for (int64_t linear = 0; linear < total; ++linear) {
                    ...
                    if (val > dst[out_offset]) dst[out_offset] = val;
                }
            } else if (min_op) {
                float* dst = (float*)out->data;
                const float* src_data = (const float*)src->data;
...
REGISTER_CPU_KERNEL(scatter_reduce, INSIGHT_DTYPE_F32, scatter_reduce_kernel_cpu);
REGISTER_CPU_KERNEL(scatter_reduce, INSIGHT_DTYPE_F64, scatter_reduce_kernel_cpu);
REGISTER_CPU_KERNEL(scatter_reduce, INSIGHT_DTYPE_I32, scatter_reduce_kernel_cpu);
REGISTER_CPU_KERNEL(scatter_reduce, INSIGHT_DTYPE_I64, scatter_reduce_kernel_cpu);
