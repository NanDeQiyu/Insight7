...
template<typename T>
static void argpartition_1d_impl(const T* src, int64_t* dst, int64_t n, int64_t kth) {
...
template<typename T>
static void argpartition_nd_impl(
    const T* src,
    int64_t* dst,
    int64_t ndim,
    const int64_t* dims,
...
#define ARGPARTITION_DISPATCH(CTYPE) \
    do { \
        if (ndim == 1) { \
            argpartition_1d_impl<CTYPE>((const CTYPE*)x->data, (int64_t*)out->data, n, kth); \
        } else { \
            argpartition_nd_impl<CTYPE>( \
                (const CTYPE*)x->data, (int64_t*)out->data, \
...
extern "C" {
    ...
    C_Status argpartition_kernel_cpu(void** inputs, void** outputs) {
        ...
        switch (x->dtype) {
        case INSIGHT_DTYPE_BOOL:   ARGPARTITION_DISPATCH(bool); break;
        case INSIGHT_DTYPE_U8:     ARGPARTITION_DISPATCH(uint8_t); break;
        case INSIGHT_DTYPE_U16:    ARGPARTITION_DISPATCH(uint16_t); break;
        case INSIGHT_DTYPE_U32:    ARGPARTITION_DISPATCH(uint32_t); break;
        case INSIGHT_DTYPE_U64:    ARGPARTITION_DISPATCH(uint64_t); break;
        case INSIGHT_DTYPE_I8:     ARGPARTITION_DISPATCH(int8_t); break;
        case INSIGHT_DTYPE_I16:    ARGPARTITION_DISPATCH(int16_t); break;
        case INSIGHT_DTYPE_I32:    ARGPARTITION_DISPATCH(int32_t); break;
        case INSIGHT_DTYPE_I64:    ARGPARTITION_DISPATCH(int64_t); break;
        case INSIGHT_DTYPE_F16:    ARGPARTITION_DISPATCH(uint16_t); break;
        case INSIGHT_DTYPE_BF16:   ARGPARTITION_DISPATCH(uint16_t); break;
        case INSIGHT_DTYPE_F32:    ARGPARTITION_DISPATCH(float); break;
        case INSIGHT_DTYPE_F64:    ARGPARTITION_DISPATCH(double); break;
        case INSIGHT_DTYPE_F8_E4M3: ARGPARTITION_DISPATCH(uint8_t); break;
        case INSIGHT_DTYPE_F8_E5M2: ARGPARTITION_DISPATCH(uint8_t); break;
        default:
...
REGISTER_CPU_KERNEL(argpartition, INSIGHT_DTYPE_F8_E4M3, argpartition_kernel_cpu);
REGISTER_CPU_KERNEL(argpartition, INSIGHT_DTYPE_F8_E5M2, argpartition_kernel_cpu);