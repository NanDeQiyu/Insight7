...
template <>
void partition_1d_impl<bool>(const bool *src, bool *dst, int64_t n,
                             int64_t kth) {
...
template <typename T>
static void partition_nd_impl(const T *src, T *dst, int64_t ndim,
...
#define PARTITION_DISPATCH(CTYPE)                                              \
  do {                                                                         \
    if (ndim == 1) {                                                           \
      partition_1d_impl<CTYPE>((const CTYPE *)x->data, (CTYPE *)out->data, n,  \
                               kth);                                           \
    } else {                                                                   \
      partition_nd_impl<CTYPE>((const CTYPE *)x->data, (CTYPE *)out->data,     \
...
C_Status partition_kernel_cpu(void **inputs, void **outputs) {
  ...
  case INSIGHT_DTYPE_BOOL:
    PARTITION_DISPATCH(bool);
    break;
  case INSIGHT_DTYPE_U8:
    PARTITION_DISPATCH(uint8_t);
    break;
  case INSIGHT_DTYPE_U16:
    PARTITION_DISPATCH(uint16_t);
    break;
  case INSIGHT_DTYPE_U32:
    PARTITION_DISPATCH(uint32_t);
    break;
  case INSIGHT_DTYPE_U64:
    PARTITION_DISPATCH(uint64_t);
    break;
  case INSIGHT_DTYPE_I8:
    PARTITION_DISPATCH(int8_t);
    break;
  case INSIGHT_DTYPE_I16:
    PARTITION_DISPATCH(int16_t);
    break;
  case INSIGHT_DTYPE_I32:
    PARTITION_DISPATCH(int32_t);
    break;
  case INSIGHT_DTYPE_I64:
    PARTITION_DISPATCH(int64_t);
    break;
  ...
  case INSIGHT_DTYPE_F32:
    PARTITION_DISPATCH(float);
    break;
  case INSIGHT_DTYPE_F64:
    PARTITION_DISPATCH(double);
    break;
