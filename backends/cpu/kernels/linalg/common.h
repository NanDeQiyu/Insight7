 ...
 */
    static inline void cpu_rowmajor_to_colmajor_f32(const float* src, float* dst, int rows, int cols) {
#pragma omp parallel for
     ...
     */
    static inline void cpu_rowmajor_to_colmajor_f64(const double* src, double* dst, int rows, int cols) {
#pragma omp parallel for
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                dst[i + j * rows] = src[i * cols + j];
     ...
     */
    static inline void cpu_colmajor_to_rowmajor_f32(const float* src, float* dst, int rows, int cols) {
#pragma omp parallel for
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
     ...
     */
    static inline void cpu_colmajor_to_rowmajor_f64(const double* src, double* dst, int rows, int cols) {
#pragma omp parallel for
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                dst[i * cols + j] = src[i + j * rows];
...
static inline void cpu_check_lapack_info(int info, const char* func_name) {
    if (info < 0) {
        char msg[256];
        snprintf(msg, sizeof(msg), "%s: illegal value at argument %d", func_name, -info);
        cpu_set_last_error(msg);
    } else if (info > 0) {
        char msg[256];
        snprintf(msg, sizeof(msg), "%s: singular/not positive definite (U(%d,%d) is zero)", func_name, info, info);
        cpu_set_last_error(msg);
    }
}
...
static inline int cpu_ensure_contiguous(InsightArray* arr) {
    if (insight_array_is_contiguous(arr)) return 1;
