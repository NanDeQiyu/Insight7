...
extern "C" {
    ...
    static void eigh_f32(const float* src, float* vals, float* vecs, int n, int uplo) {
    ...
    static void eigh_f64(const double* src, double* vals, double* vecs, int n, int uplo) {
...
C_Status eigh_kernel_cpu(void** inputs, void** outputs) {
    ...
    if (!cpu_ensure_contiguous(x) || !cpu_ensure_contiguous(vals) || !cpu_ensure_contiguous(vecs))
        return C_FAILED;
    ...
    if (x->dtype == INSIGHT_DTYPE_F32) {
        eigh_f32((float*)x->data, (float*)vals->data, (float*)vecs->data, n, uplo);
    } else {
        eigh_f64((double*)x->data, (double*)vals->data, (double*)vecs->data, n, uplo);
    }
    return C_SUCCESS;
}
