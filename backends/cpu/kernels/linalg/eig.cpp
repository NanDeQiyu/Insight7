...
extern "C" {
    ...
    static void eig_f32(const float* src, float* vals_real, float* vals_imag,
        ...
        char jobvl[2] = "N";
        char jobvr[2] = "V";
        sgeev_(jobvl, jobvr, &n, a, &n, wr, wi, NULL, &n, vr, &n, work, &lwork, &info);
        lwork = (int)work[0];
    ...
    static void eig_f64(const double* src, double* vals_real, double* vals_imag,
        ...
        char jobvl[2] = "N";
        char jobvr[2] = "V";
        dgeev_(jobvl, jobvr, &n, a, &n, wr, wi, NULL, &n, vr, &n, work, &lwork, &info);
        lwork = (int)work[0];
...
C_Status eig_kernel_cpu(void** inputs, void** outputs) {
    ...
    if (x->dtype == INSIGHT_DTYPE_F32) {
        eig_f32((float*)x->data, (float*)vals_real->data, (float*)vals_imag->data,
                (float*)vecs_real->data, (float*)vecs_imag->data, n);
    } else {
        eig_f64((double*)x->data, (double*)vals_real->data, (double*)vals_imag->data,
                (double*)vecs_real->data, (double*)vecs_imag->data, n);
    }
    return C_SUCCESS;
}
