...
C_Status lu_unpack_kernel_cpu(void **inputs, void **outputs) {
  ...
  if (LU->dtype == INSIGHT_DTYPE_F32) {
    lu_unpack_f32((float *)LU->data, ipiv, (float *)P_arr->data,
                  (float *)L_arr->data, (float *)U_arr->data, n);
  } else {
    lu_unpack_f64((double *)LU->data, ipiv, (double *)P_arr->data,
                  (double *)L_arr->data, (double *)U_arr->data, n);
  }
  return C_SUCCESS;
}
