...
// 对外接口
void qr_f64_impl(const double* src, double* q, double* r, int m, int n, int mode) {
...
extern "C" {
    ...
    C_Status qr_kernel_cpu(void** inputs, void** outputs) {
        ...
        if (x->dtype == INSIGHT_DTYPE_F64) {
            qr_f64_impl((double*)x->data, (double*)Q->data, (double*)R->data, m, n, mode);
        }
        else if (x->dtype == INSIGHT_DTYPE_F32) {
            qr_f32_impl((float*)x->data, (float*)Q->data, (float*)R->data, m, n, mode);
        }
        else {
