...
static void matmul_f64_scalar_fallback(const double* a, const double* b, double* c,
                                        int m, int k, int n) {
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            double sum = 0.0;
...
static void matmul_f64_vector_fallback(const double* a, const double* b, double* c,
                                       int m, int n, int is_a_vec) {
    if (is_a_vec) {
        for (int j = 0; j < n; ++j) {
            double sum = 0.0;
...
static void matmul_f64_matrix_matrix(const double* a, const double* b, double* c,
                                     int m, int k, int n) {
#if HAVE_BLAS_ACCEL
...
static void matmul_f64_matrix_vector(const double* a, const double* b, double* c,
                                     int m, int n, int is_a_vec) {
#if HAVE_BLAS_ACCEL
    if (is_a_vec) {
...
static double matmul_f64_vector_vector(const double* a, const double* b, int n) {
#if HAVE_BLAS_ACCEL
...
C_Status matmul_kernel_cpu(void** inputs, void** outputs) {
    ...
    if (ndim_a == 2 && ndim_b == 1) {
        ...
        if (out->dtype == INSIGHT_DTYPE_F32) {
            matmul_f32_matrix_vector((float*)a->data, (float*)b->data, (float*)out->data,
                                     m, n, 0);
        } else {
            matmul_f64_matrix_vector((double*)a->data, (double*)b->data, (double*)out->data,
                                     m, n, 0);
        }
        return C_SUCCESS;
    }
    ...
    if (ndim_a == 1 && ndim_b == 2) {
        ...
        if (out->dtype == INSIGHT_DTYPE_F32) {
            matmul_f32_matrix_vector((float*)b->data, (float*)a->data, (float*)out->data,
                                     m, n, 1);
        } else {
            matmul_f64_matrix_vector((double*)b->data, (double*)a->data, (double*)out->data,
                                     m, n, 1);
        }
        return C_SUCCESS;
    }
    ...
    if (ndim_a == 2 && ndim_b == 2) {
        ...
        if (out->dtype == INSIGHT_DTYPE_F32) {
            matmul_f32_matrix_matrix((float*)a->data, (float*)b->data, (float*)out->data,
                                     m, k, n);
        } else {
            matmul_f64_matrix_matrix((double*)a->data, (double*)b->data, (double*)out->data,
                                     m, k, n);
        }
        return C_SUCCESS;
    }
    ...
    // At this point, we have broadcasted shapes; we can flatten the batch dimensions.
    int64_t batch = 1;
    ...
    if (out->dtype == INSIGHT_DTYPE_F32) {
        ...
        matmul_f64_batched((double*)a->data, (double*)b->data, (double*)out->data,
                           batch, m, k, n);
    }
    return C_SUCCESS;
}
