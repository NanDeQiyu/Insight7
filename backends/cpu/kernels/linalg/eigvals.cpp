...
extern "C" {
    ...
    static void eigvals_f32(const float* src, float* vals_real, float* vals_imag, int n) {
        ...
        char jobvl[2] = "N";
        char jobvr[2] = "N";
        sgeev_(jobvl, jobvr, &n, a, &n, wr, wi, NULL, &n, NULL, &n, work, &lwork, &info);
        lwork = (int)work[0];
        ...
        free(a); free(wr); free(wi); free(work);
    }

    static void eigvals_f64(const double* src, double* vals_real, double* vals_imag, int n) {
        ...
        char jobvl[2] = "N";
        char jobvr[2] = "N";
        dgeev_(jobvl, jobvr, &n, a, &n, wr, wi, NULL, &n, NULL, &n, work, &lwork, &info);
        lwork = (int)work[0];
...
C_Status eigvals_kernel_cpu(void** inputs, void** outputs) {
    ...
    if (x->dtype == INSIGHT_DTYPE_F32) {
        eigvals_f32((float*)x->data, (float*)out_real->data, (float*)out_imag->data, n);
    } else {
        eigvals_f64((double*)x->data, (double*)out_real->data, (double*)out_imag->data, n);
    }
    return C_SUCCESS;
}
