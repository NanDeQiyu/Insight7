...
template<typename T>
static void inv_general(const T* src, T* dst, int n);

// 特化版本
template<>
void inv_general<float>(const float* src, float* dst, int n) {
    inv_f32(src, dst, n);
}

template<>
void inv_general<double>(const double* src, double* dst, int n) {
    inv_f64(src, dst, n);
}
