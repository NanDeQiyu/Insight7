...
extern "C" {
    ...
    static void solve_f32(const float* A, const float* B, float* X,
        int n, int nrhs) {
    ...
    static void solve_f64(const double* A, const double* B, double* X,
        int n, int nrhs) {
        // 将 A 从行主序转换为列主序（LAPACK 期望列主序）
...
C_Status solve_kernel_cpu(void** inputs, void** outputs) {
    ...
    if (A->dtype == INSIGHT_DTYPE_F32) {
        ...
        solve_f64((double*)A->data, (double*)B->data, (double*)out->data, n, nrhs);
    }
    return C_SUCCESS;
}
