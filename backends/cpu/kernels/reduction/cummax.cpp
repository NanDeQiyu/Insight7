 ...
 */
template <typename T>
static void cummax_impl(const T *src, T *dst, int64_t src_offset,
                        int64_t dst_offset, int64_t axis_size,
...
#pragma omp parallel for
  for (int64_t outer = 0; outer < outer_stride; ++outer) {
    for (int64_t inner = 0; inner < inner_stride; ++inner) {
      ...
      int64_t src_base =
          src_offset + outer * src_stride_outer + inner * src_stride_inner;
      int64_t dst_base =
          dst_offset + outer * dst_stride_outer + inner * dst_stride_inner;
 ...
 */
static int64_t compute_outer_stride(int64_t ndim, const int64_t *strides,
                                    int axis, int64_t inner_stride) {
 ...
 */
static int64_t compute_axis_stride(int64_t ndim, const int64_t *strides,
                                   int axis) {
  return strides[axis];
}
 ...
 */
static int64_t compute_inner_stride(int64_t ndim, const int64_t *strides,
                                    int axis) {
  if (axis + 1 >= ndim) {
    return 1;
  }
...
#define CUMMAX_CASE(DTYPE, CTYPE)                                              \
  case DTYPE:                                                                  \
    cummax_impl<CTYPE>((const CTYPE *)x->data, (CTYPE *)out->data, x->offset,  \
                       out->offset, axis_size, outer_stride, inner_stride,     \
                       src_stride_outer, dst_stride_outer, src_stride_axis,    \
                       dst_stride_axis, src_stride_inner, dst_stride_inner);   \
    break;
...
C_Status cummax_kernel_cpu(void **inputs, void **outputs) {
  ...
  int64_t src_stride_outer =
      compute_outer_stride(ndim, x->strides, axis, inner_stride);
  int64_t dst_stride_outer =
      compute_outer_stride(ndim, out->strides, axis, inner_stride);
