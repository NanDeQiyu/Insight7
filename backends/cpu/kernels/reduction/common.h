 ...
 * Input arrays are guaranteed to be contiguous (batch_size 脳 reduce_size layout)
 * by the frontend preparation step.
 */
...
static inline bool cpu_is_nan_f32(float x) {
    return isnan(x);
}

static inline bool cpu_is_nan_f64(double x) {
    return isnan(x);
}
