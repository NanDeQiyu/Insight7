...
template<typename T>
static void cumsum_impl(
    const T* src, T* dst,
    int64_t axis_size, int64_t outer_stride, int64_t inner_stride) {
...
extern "C" {
    ...
    C_Status cumsum_kernel_cpu(void** inputs, void** outputs) {
        ...
        case INSIGHT_DTYPE_F32:
            cumsum_impl<float>((const float*)x->data, (float*)out->data,
                axis_size, outer_stride, inner_stride);
            break;
        case INSIGHT_DTYPE_F64:
            cumsum_impl<double>((const double*)x->data, (double*)out->data,
                axis_size, outer_stride, inner_stride);
            break;
