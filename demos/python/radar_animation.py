...
def ca_cfar_2d(energy, guard=(4, 4), ref=(12, 12), pfa=1e-6):
    ...
    total_r = g_r + ref[1]
    big = uniform_filter(
        energy.astype(np.float64), size=(2 * total_d + 1, 2 * total_r + 1), mode="nearest"
    )
    small = uniform_filter(
        energy.astype(np.float64), size=(2 * g_d + 1, 2 * g_r + 1), mode="nearest"
    )
...
def save_frame(
    frame_idx, pc_all, energy, range_bins, doppler_bins, targets, delays, output_dir, dpi=100
):
    """生成四子图并保存为 PNG。"""
    ...
    if targets:
        ...
        ax.scatter(rs, ds, c="red", s=200, marker="x", linewidths=3, label="Targets")
...
def main():
    ...
    parser.add_argument("--frames", type=int, default=200, help="帧数 (默认: 200)")
    parser.add_argument("--output", type=str, default=FRAMES_DIR, help="输出目录")
    parser.add_argument("--dpi", type=int, default=100, help="图片 DPI (默认: 100, 降低可加速)")
    parser.add_argument(
        "--pulses", type=int, default=N_PULSES, help="每帧脉冲数 (默认: 32, 降低可加速)"
    )
    ...
    for frame_idx in range(n_frames):
        ...
        print(
            f"  [{frame_idx+1:3d}/{n_frames}] {elapsed:.2f}s "
    ...
    print("        设置每页自动切换 0.1s 即可看到雷达动画效果")
    print(f"{'=' * 60}")
