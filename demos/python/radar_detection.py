...
def run_detection(
    ...
    # [1] 回波模拟 (全 GPU 向量化)
    t0 = time.time()
    with ins.ProfileBlock(profiler, "echo_simulation"):
        pulses = _simulate_echoes(
            target_delays, target_dopplers, external_noise_r, external_noise_i
        )
    t_echo = time.time() - t0

    # [2] 脉冲压缩 (框架 API, batched FFT)
    t0 = time.time()
    with ins.ProfileBlock(profiler, "pulse_compression"):
    ...
    # [3] 去直流 + 多普勒处理 (框架 API)
    t0 = time.time()
    with ins.ProfileBlock(profiler, "doppler"):
    ...
    # [5] CA-CFAR 检测
    t0 = time.time()
    with ins.ProfileBlock(profiler, "ca_cfar"):
        _, det = ins.signal.ca_cfar(energy, [4, 4], [12, 12], 1e-6)
    t_cfar = time.time() - t0

    # [6] 目标提取 (批量 GPU→CPU 传输)
    t0 = time.time()
    with ins.ProfileBlock(profiler, "target_extraction"):
...
if __name__ == "__main__":
    ...
    # 创建 Profiler (ambgfun 已完成, Profiler 仅用于帧循环)
    ...
    for frame in range(n_frames):
        ...
        if device_all:
            ...
            result = run_detection(
                args.device, seed_val, frame, n_frames, profiler=prof if args.profiler else None
            )
            t = result["total_ms"]
            if args.device == "gpu":
                times_gpu.append(t)
